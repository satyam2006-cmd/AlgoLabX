...
const Header = ({ setActiveTab, setSelectedAlgorithm }) => {
  ...
  return (
    <motion.div
      ...
      className="h-16 bg-dark-900/80 border-b border-dark-700/40 flex items-center justify-between px-6 sticky top-0 z-50 backdrop-blur-xl"
    >
      {/* Left Section - Title */}
      ...
      <div className="max-w-md w-full px-8 relative">
        <div className="w-full relative group">
          <input
            ...
            className="w-full bg-dark-800/50 border border-dark-700/50 rounded-xl py-2.5 pl-10 pr-4 text-dark-200 text-sm placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-dark-500/50 transition-all duration-300 group-hover:bg-dark-800/80"
          />
          <svg
        ...
        <AnimatePresence>
          {showSuggestions && filteredSuggestions.length > 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }}
              className="absolute top-full left-8 right-8 mt-2 bg-dark-900 border border-dark-700/60 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden z-[100] backdrop-blur-2xl"
            >
              {filteredSuggestions.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {item.type === 'algo' ? (
                      <span className="px-2 py-0.5 rounded-md bg-dark-800 text-[10px] text-dark-400 border border-dark-700/30">Algorithm</span>
                    ) : (
      ...
      <div className="flex items-center gap-3">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="w-10 h-10 rounded-xl bg-dark-800/50 border border-dark-700/50 flex items-center justify-center text-dark-400 hover:text-dark-200 transition-colors"
        >
        ...
        <motion.div
          whileHover={{ scale: 1.05 }}
          className="w-10 h-10 rounded-xl bg-gradient-to-br from-dark-500 to-dark-700 flex items-center justify-center text-dark-50 font-bold border border-dark-500/30 shadow-lg cursor-pointer"
        >
          U
        </motion.div>
      </div>
